...
class PGA_GPP_Simulator:
    ...
    def generate_lineups(
        ...
        min_salary = np.quantile(salaries, 0.5)
        lus = {}
    ...
    def output(self):
        ...
        # Sort the players in the lineup by salary (descending) and then by their name (ascending)
            sorted_lineup = sorted(x["Lineup"], key=lambda player: (-self.player_dict[player]["Salary"], player))
            ...
            if self.site == "dk":
                if self.use_contest_data:
                    ...
                    # Simplify the lineup_str by iterating through the sorted_lineup
                    players_str_parts = ["{} ({})".format(player.replace("#", "-"), self.player_dict[player]["ID"]) for player in sorted_lineup]
                    players_str = ','.join(players_str_parts)
                    lineup_str = "{},{},{},{},{}%,{}%,{}%,{},${},{}".format(
                        players_str, fpts_p, ceil_p, salary, win_p, top10_p, roi_p, own_p, roi_round, lu_type
                )
                else:
                    players_str_parts = ["{} ({})".format(player.replace("#", "-"), self.player_dict[player]["ID"]) for player in sorted_lineup]
                    players_str = ','.join(players_str_parts)
                    lineup_str = "{},{},{},{},{}%,{}%,{},{}%,{}".format(
                        players_str, fpts_p, ceil_p, salary, win_p, top10_p, own_p, cash_p, lu_type
                )
            elif self.site == "fd":
                if self.use_contest_data:
                    roi_p = round(
